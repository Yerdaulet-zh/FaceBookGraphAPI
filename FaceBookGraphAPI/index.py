...
async def parse_data(competitor):
    ...
    response = requests.get(url=params['parse_url']).json()
    is_end = await asyncio.create_task(save_response(response, competitor))
    return response, params, is_end


async def pagination(response, competitor, params):

    query = '{' + f"media.after({response['business_discovery']['media']['paging']['cursors']['after']})" + '{id,caption,comments_count,like_count,media_type,timestamp},followers_count,media_count}'
    response = requests.get(params['base'] + params['insta_acc_id'] + f"?fields=business_discovery.username({competitor}){query}&access_token={params['access_token']}").json()
    is_end = await asyncio.create_task(save_response(response, competitor))
    return response, is_end


async def save_response(response, competitor):
    """
    ...
    try:
        ...
        for post in posts:
            ...
            post['followers_count'] = response['business_discovery']['followers_count']
            post['media_count'] = response['business_discovery']['media_count']
